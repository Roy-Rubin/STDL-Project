...
def parse_cli():
    ...
    sp_dist = sp.add_parser('prepare-dist',
                            help='Prepare homework for distribution to '
                                 'students')
    sp_dist.set_defaults(subcmd_fn=prepare_dist)
    sp_dist.add_argument('--hw-dir', '-i', type=is_dir, help='hw folder',
                         required=True)
    sp_dist.add_argument('--out-dir', '-o', type=str, help='Output folder',
                         default='./out', required=False)

    # Prepare submission
    sp_subm = sp.add_parser('prepare-submission',
                            help='Prepare homework for submission')
    sp_subm.set_defaults(subcmd_fn=prepare_submission)
    sp_subm.add_argument('--hw-dir', type=is_dir, help='hw folder',
                         default='.', required=False)
    sp_subm.add_argument('--out-dir', '-o', type=is_dir, help='Output folder',
                         default='.', required=False)
    sp_subm.add_argument('--id', '-i', type=int, help='Submitter id',
                         action='append', metavar='ID',
                         required=True, dest='submitter_ids')
    sp_subm.add_argument('--skip-run', '-R', action='store_true',
                         help='Skip running notebooks', required=False)
    sp_subm.add_argument('--allow-errors', '-E', action='store_true',
                         help='Allow errors when running notebooks',
                         required=False)

    # Clear notebook outputs
    sp_clear = sp.add_parser('clear-nb',
                             help='clear outputs from notebooks')
    sp_clear.set_defaults(subcmd_fn=clear_notebooks)
    sp_clear.add_argument('nb_paths', type=is_file, help='notebooks to run',
                          metavar='NB_PATH', nargs='+')

    # Run notebooks
    sp_run = sp.add_parser('run-nb', help='run jupyter notebooks')
    sp_run.set_defaults(subcmd_fn=run_notebooks)
    sp_run.add_argument('nb_paths', type=is_file, help='notebooks to run',
                        metavar='NB_PATH', nargs='+')
    sp_run.add_argument('--allow-errors', '-E', action='store_true',
                        help='Allow errors when running notebooks',
                        required=False)
...
def run_notebooks(nb_paths, allow_errors=False, **kwargs):
    ...
    for nb_path in nb_paths:
        try:
            jupyter_utils.nbconvert(nb_path, execute=True, inplace=True,
        ...
        for root, dirs, files in os.walk(dest_dir):
            ...
            for py_file in py_files:
                ...
                with open(py_file, 'r') as file_handle:
                    ...
                    new_content, n_subs_code, n_subs_answers = \
                        answers.clear_solutions(content)
