...
def runExperiment(ds_train : Dataset, ds_test : Dataset, hyperparams, device, model_name, dataset_name):
    ...
    **runExperimentWithModel_BasicConvNet**
    ...
    if dataset_name.startswith("single_gene"):
        ...
        # TODO: uncomment this next block later !! itsis important !
...
def getFullDimsPrediction_with_AE_DS(dataset, model, device):
    ...
    for batch_index in range(num_of_batches):
        with torch.no_grad():  # no need to keep track of gradients since we are only using the forward of the model
            ...
            '''
            y_pred_decoded = dataset.autoEncoder.decodeWrapper(y_pred)
    ...
    y_pred_final = torch.from_numpy(y_pred_final).cuda()  # change back to a tensor in order to insert to AE net
    M_pred = dataset.autoEncoder.decodeWrapper(y_pred_final) # TODO: might need to put .to(device) here or .cpu().detach() here
